...
def ReceiptDetailsView():
    ...
    return render_template('receipt-details.html'
...
def CreateReceipt(id):
    ...
    lapphieukham = utils.get_info_medical_form()
    p = dao.lenhoadon(id)
    m = dao.get_medical_form_id(id)
    int(p)
    return render_template('create-receipt.html', p=p
                           , lapphieukham=lapphieukham, tienkham=tienkham, m=m)
...
def Payment():
    if request.method.__eq__('POST'):
        ...
        examines_price = request.args.get('examines_price')
        total = request.args.get('total')
        patient_id =request.args.get('patient_id')

        try:
            dao.add_receipt(patient_id=patient_id,examines_price=examines_price,total=total)
        except:
            err_msg = "Lỗi"
        else:
